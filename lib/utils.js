...
function removeUniqSelectName(type, arr) {
    ...
    function empty(name) { return name ? !name.trim() : name; }
    function hcName(name) { return name ? name.startsWith("_") : name; }
}
...
function makeUniqString(type, v) {
    ...
    switch (type) {
        case 'logs':
            // 包含非精确ID合并  使用 ${v.$CallLog.duration} 代替  ${v.ms}
            return `${str}-${v.$CallLog.duration}-${num}`;
        case 'sms':
            return `${str}-${v.ms}-${num}`;
        default:
            throw new Error('unknown type', type);
